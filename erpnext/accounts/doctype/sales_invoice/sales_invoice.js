...
erpnext.accounts.SalesInvoiceController = erpnext.selling.SellingController.extend({
    ...
    refresh: function(frm,doc, dt, dn) {        
        this._super();
        //CustomeScriptWork
        if (!this.frm.doc.name.startsWith("New Sales Invoice") &&
           (this.frm.doc.structured_communication == undefined || this.frm.doc.structured_communication == null)) {
            var invoiceNumber = null;
            invoiceNumber = this.frm.doc.name;

            var mod = parseInt(invoiceNumber) % 97;
            ...
            if (mod === 0) {
                ...
                if (mod < 10) {
                    invoiceNumber = invoiceNumber + "0" + mod.toString();
                }
                else {
                    invoiceNumber = invoiceNumber + mod.toString();
                }
            }
            var formatted_invoiceNumber = "+++" + invoiceNumber.substring(0, 3) + "/" + invoiceNumber.substring(3, 7) + "/" + invoiceNumber.substring(7) + "+++";
            this.frm.doc.structured_communication = formatted_invoiceNumber;
            refresh_field("structured_communication");

            if (!this.frm.doc.name.startsWith("New Sales Invoice") &&
            (this.frm.doc.structured_communication != undefined || this.frm.doc.structured_communication != null)) {
                var name = this.frm.doc.name,
                    value = this.frm.doc.structured_communication;
                setTimeTrackingStatus("Sales Invoice", name, 'structured_communication', value, function (status) {                    
                });                
            }
        ...
        if (this.frm.doc.contract_name == undefined) {
        ...
        else if (this.frm.doc.contract_name != undefined) {
            cur_frm.set_df_property("contract_section", "hidden", false);
            var monthly_amount = 0;
            //var periodic_amount = 0;
            $.each(this.frm.doc.items, function (index, item) {
                if (item.item_name != "Periodic Balance Amount") {
            ...
            this.frm.doc.monthly_amount = monthly_amount;
            refresh_field("monthly_amount");
        ...
        if (this.frm.doc.posting_date == undefined) {
    ...
    on_submit: function (frm) {
        ...
        var ttdList = tempList || [];

        for (tt in ttdList) {
            var ttItem = ttdList[tt];
            updatetimetracking(ttItem.name, "Invoiced");
        }
    ...
    before_save: function (frm, doc) {
        var ttdList = tempList || [];
        for (tt in ttdList) {
            var ttItem = ttdList[tt];
            updatetimetracking(ttItem.name, "Submitted");
        }
    },
    //EndOfCustomeScriptWork
...
//CustomeScriptWork
function updatetimetracking(item, itemstatus) {

    setTimeTrackingStatus("TimeTracking", item, 'status', itemstatus, function (status) {

    });
}


setTimeTrackingStatus = function (doctype, name, status, value, func) {
    frappe.call({
...
getPaymentFrequencyData = function (frm, func) {
    
    var payment_frequency = frm.doc.payment_frequency;
    if (payment_frequency != undefined && payment_frequency != "") {
        frappe.call({
